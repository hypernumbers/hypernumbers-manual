=======================
Vixo Template Functions
=======================

Template functions enable you to put buttons on web/wiki pages that enable users to create new pages from templates, but under your control. The following buttons are supported:

.. toctree::
   :maxdepth: 1

   CREATE.BUTTON       <create-button>
   CREATE.BUTTON.IF    <create-button-if>
   MAP.ROWS.BUTTON     <map-rows-button>
   MAP.ROWS.BUTTON.IF  <map-rows-button-if>
   MAP.SHEET.BUTTON    <map-sheet-button>
   MAP.SHEET.BUTTON.IF <map-sheet-button-if>

